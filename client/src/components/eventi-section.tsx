 ...
 <div className="flex gap-6 overflow-x-auto scrollbar-hide py-2">
  {displayedEvents && displayedEvents.length > 0 ? (
    displayedEvents.map((event) => (
      <Card
        key={event.id}
        className={`flex-shrink-0 w-[250px] bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 ${
          showPast ? 'opacity-75' : ''
        }`}
      >
        <img
          src={
            event.posterUrl ||
            "https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=250"
          }
          alt={event.title}
